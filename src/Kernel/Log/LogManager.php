...
namespace Kolrank\Sdk\Api\Kernel\Log;
...
class LogManager implements LoggerInterface
{
    ...
    {
        return new Monolog('KolRank', $this->prepareHandlers([new StreamHandler(
            \sys_get_temp_dir().'/kolrank/kolrank.log', $this->level(['level' => 'debug'])
        )]));
    }
    ...
    protected function createSyslogDriver(array $config)
    {
        return new Monolog($this->parseChannel($config), [
            $this->prepareHandler(new SyslogHandler(
                    'KolRank', $config['facility'] ?? LOG_USER, $this->level($config))
            ),
        ]);
    }
